...
class FreeplayState extends MusicBeatSubstate
{
	...
	override function create():Void
	{
		...
		fnfFreeplay.font = OldPaths.font("vcr.ttf");
		...
		if (controls.FAVORITE #if TOUCH_CONTROLS_ALLOWED || touchPad?.buttonF.justPressed #end && !busy) // ? change control binding
		{
			...
			if (targetSong != null)
			{
				...
				else
				{
					...
					FunkinSound.playOnce(Paths.sound('unfav'), ClientPrefs.data.sfxVolume);
	...
	public function playCurSongPreview(?daSongCapsule:SongMenuItem):Void
	{
		...
		if (curSelected != 0)
		{
			...
			isLoadedInst = FunkinSound.playMusic(daSongCapsule.songData.getNativeSongId(), {
				startingVolume: 0.0,
				overrideExisting: true,
				restartTrack: false,
				pathsFunction: INST,
